...
public final class CategoryBrowser {
  ...
  private final class CategoryForm {
    ...
    private void createForm() {
      ...
      form.setCaption(getI18nProvider().getMessage("categories.form.caption"));
      ...
      final Button saveButton = new Button("Save", new Button.ClickListener() {
        ...
        public void buttonClick(ClickEvent event) {
          /*
           * First, we have to commit the form. This will validate the data and
           * update the underlying category instance upon success.
           */
          try {
            form.commit();
          } catch (Exception e) {
            /*
             * The form contains errors, so we abort here and let the user fix
             * them. The form will take care of displaying the error messages to
             * the user.
             */
            return;
          }
          /*
           * We now have a category instance with the updated data, so now we
           * have to save it using the category service.
           */
          try {
            if (isNew) {
            ...
            /*
             * Remember to refresh the container, otherwise the new/updated
             * category won't show up in the tree.
             */
            categoryContainer.refresh();
            /*
             * Update selection, will update the enablement state
             */
            categoryTree.setValue(category.getId());

            setVisible(false);
            getComponent().getWindow().showNotification(
                getI18nProvider().getMessage("categories.categorySaved"),
                Notification.TYPE_TRAY_NOTIFICATION);
          } catch (AccessDeniedException e) {
            if (logger.isDebugEnabled()) {
              logger.debug("Access denied while attempting to save category ["
                  + category + "]", e);
            }
            if (isNew) {
            ...
            if (logger.isErrorEnabled()) {
              logger.error("Error while attempting to save category ["
                  + category + "]", e);
            }
     ...
     * 
     * @param parent
     *          the parent of the category, <code>null</code> to make a new root
     *          category.
     */
     ...
     * 
     * @param visible
     *          <code>true</code> to show the form, <code>false</code> to hide
     *          it.
     */
  ...
  /**
   * Listener interface to be implemented by classes that want to know when the
   * currently selected category is changed.
   * 
  ...
  public static interface CategorySelectionListener {
     ...
     * 
     * @param newCategory
     *          the selected category, or <code>null</code> if no category is
     *          selected at all.
     */
   ...
   *          the category service to use for fetching and modifying categories
   *          (must not be <code>null</code>).
   * @param i18nProvider
   *          the i18n provider to use for fetching localized strings (must not
   *          be <code>null</code>).
   */
  ...
  /**
   * Shows the access control list for the selected category. If no category is
  ...
  /**
   * Shows the audit log for the selected category. If no category is selected,
  ...
  /**
   * Deletes the selected category. If no category is selected, this method does
   * nothing.
   */
  private void actionDelete() {
    ...
    if (selectedCategoryId != null) {
      if (logger.isDebugEnabled()) {
        logger.debug("Attempting to delete category '" + selectedCategoryId
            + "'");
      }
      try {
        ...
        /*
         * Remember to refresh the container, otherwise the old category will
         * remain in the tree.
         */
        categoryContainer.refresh();
        /*
        ...
        if (logger.isDebugEnabled()) {
          logger.debug("Access denied while attempting to delete category '"
              + selectedCategoryId + "'", e);
        }
        ExceptionUtils.handleException(getComponent().getWindow(), e,
            getI18nProvider()
                .getMessage("categories.delete.accessDenied.descr"));
      } catch (OptimisticLockingFailureException e) {
        if (logger.isDebugEnabled()) {
          logger.debug(
        ...
        ExceptionUtils
            .handleException(getComponent().getWindow(), e, getI18nProvider()
                .getMessage("categories.delete.optLockFail.descr"));
      } catch (Exception e) {
        if (logger.isErrorEnabled()) {
  ...
  private void actionRefresh() {
    try {
      ...
      if (logger.isErrorEnabled()) {
        logger.error("Error while attempting to refresh the category browser",
            e);
      }
      ExceptionUtils.handleException(getComponent().getWindow(), e);
    }
  ...
  /**
   * Adds <code>listener</code> to the list of listeners to be notified when the
   * category selection is changed. If the listener is <code>null</code>,
   * nothing happens.
   * 
   * @param listener
   *          the listener to add.
  ...
  private void createComponent() {
    ...
    /*
     * The tree will show all the categories returned from the category service.
     */
    ...
    /*
     * The form for editing categories is hidden by default and is shown when
     * the user clicks the edit or add button.
     */
    ...
    final HorizontalLayout toolbar = new HorizontalLayout();
    {
      ...
      /*
       * Button: Add a new category. The currently selected category will be
       * used as parent.
       */
      ...
      /*
       * Button: Show/edit the access control list for the selected category
       */
  ...
  /**
   * Gets the category service that should be used when retrieving or modifying
   * categories.
   * 
  ...
  /**
   * Updates the enablement state of the toolbar buttons and hides the category
   * form.
   */
  private void updateEnablementState() {
    ...
    /*
     * In a real application, security checks should be included here as well in
     * order to disable functions that the user is not allowed to perform. In
     * this demo, it is possible to try to perform illegal operations just to
     * demonstrate that the security features are working.
     */
