...
public class CategoryContainerTest {
  ...
  public void testAddContainerProperty() {
    ...
    container.addContainerProperty("propId", String.class, "blah"); // This
                                                                    // method is
                                                                    // not
                                                                    // implemented
  }
  ...
  public void testAreChildrenAllowed() {
    ...
    for (Category root : serviceStub.getRootCategories()) {
      ...
      for (Category c : serviceStub.getChildren(root)) {
        /*
         * Even though it is legal to add children to any category, this method
         * should return false for categories without children to make sure they
         * show up as leaves.
         */
  ...
  public void testGetContainerProperty() {
    ...
    for (Category root : serviceStub.getRootCategories()) {
      assertEquals(root.getId(),
          container.getContainerProperty(root.getId(), "id").getValue());
      assertEquals(root.getVersion(),
          container.getContainerProperty(root.getId(), "version").getValue());
      assertEquals(root.getName(),
          container.getContainerProperty(root.getId(), "name").getValue());
      assertEquals(root.getDescription(),
          container.getContainerProperty(root.getId(), "description")
              .getValue());
      assertNull(container.getContainerProperty(root.getId(), "parent")
          .getValue());
      for (Category c : serviceStub.getChildren(root)) {
        // Assume the rest of the properties are working
        assertEquals(c.getParent(),
            container.getContainerProperty(c.getId(), "parent").getValue());
      }
    }
  }
  ...
  public void testGetContainerProperty_NonexistentProperty() {
    System.out.println("testGetContainerProperty_NonexistentItem");
    assertNull(container.getContainerProperty(serviceStub.getRootCategories()
        .get(0).getId(), "nonexistent"));
  }
  ...
  public void testGetItem() {
    ...
    for (Category root : serviceStub.getRootCategories()) {
      ...
      assertEquals(root.getVersion(), item.getItemProperty("version")
          .getValue());
      ...
      for (Category c : serviceStub.getChildren(root)) {
        ...
        // Assume the rest of the properties are working
        assertEquals(c.getParent(), childItem.getItemProperty("parent")
            .getValue());
      }
    }
  }
